...
Server.prototype.launchServer = function () {
  ...
  var callTotoroClient = function (options, callback) {
    ...
    var serverCmd = '/home/lib/node-0.12.1/node ' + runTotoroFile
    var devCmd = 'node ' + runTotoroFile
    childProcess.exec(devCmd + ' ' + options.runner + ' ' + that.cfg.host, function (err, stdout, stderr) {
      if (err) {
        console.error(err)
      }
