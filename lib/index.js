...
Server.prototype.launchServer = function() {
  ...
  */
  ...
  app.get('/runTask', function (req, res){
    ...
    // todo, 命令不一样 cfg.host
    childProcess.exec('/home/lib/node-0.12.1/node /home/lib/totoro-server/lib/run-totoro.js ' + runner + ' ' + '119.29.8.126', function(err, stdout, stderr){
      if (err) { console.error(err) }
      var content = fs.readFileSync(path.join(__dirname, '../cache.json'))
      res.send(content)
    })
